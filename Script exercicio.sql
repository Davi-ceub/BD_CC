...
select nome_evento,count(id_evento),count(status_pagamento) from Eventos as e inner join Inscricoes as i on e.id = i.id_evento  where status_pagamento = 'pendente' group by nome_evento order by count(status_pagamento) desc;
