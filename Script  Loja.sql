...
select count(produto_id) from pedidos;
...
select nome,quantidade,produto_id from pedidos as r join produtos as p on r.produto_id = p.id order by quantidade desc;
...
select cliente_id,sum(p.preco * r.quantidade) from pedidos as r join produtos as p on r.produto_id = p.id join clientes as c on c.id = r.cliente_id group by cliente_id limit 1;
