...
select p.nome,
   ...
   else 0 end) as qtd_baixa,
   
   sum(CASE 
   when t.prioridade = 'Média' then 1 
   else 0 end) as qtd_media,
   
   sum(CASE
   when t.prioridade = 'Alta' then 1 
   else 0 end) as qtd_alta,
   
   sum(CASE 
   when t.prioridade = 'Urgente' then 1 
   else 0 end) as qtd_urgente 
   from Projetos as p inner join Tarefas as t on p.id = t.id_projeto group by p.nome order by p.nome;

-- 2) Mostre o nome dos responsáveis e a quantidade de tarefas pelas quais eles são responsáveis (id_responsavel_tarefa). 
-- Liste apenas os responsáveis que têm 3 ou mais tarefas atribuídas. Ordene do mais atarefado para o menos atarefado.
select nome,count(id_responsavel_tarefa) as qtd from Responsaveis as r join Tarefas as t on r.id = t.id_responsavel_tarefa group by nome order by count(id_responsavel_tarefa) desc limit 2;

-- 3) Calcule a "duração percebida" média das tarefas (em dias) para projetos, agrupados pelo status do projeto ('Pendente', 'Em Andamento', 'Concluído', 'Cancelado'). 
-- Para este exercício, considere a "duração percebida" de uma tarefa como a diferença em dias entre a data_prevista_entrega da tarefa e a data_inicio do projeto ao qual ela pertence. Exiba o status do projeto e a respectiva duração percebida média das tarefas. Arredonde a média para duas casas decimais. Ignore tarefas ou projetos onde as datas necessárias sejam nulas.
select p.status,avg(DATEDIFF(data_prevista_entrega,data_inicio)) as Média from Projetos as p join Tarefas as t on p.id = t.id_projeto group by p.status;

-- 4) Para cada projeto que esteja atualmente com o status 'Em Andamento', liste o nome do projeto. 
-- Dentro de cada um desses projetos, mostre cada cargo (da tabela Responsaveis) dos responsáveis que possuem tarefas atribuídas nesse projeto específico, e o número de tarefas que responsáveis com aquele cargo têm nesse projeto. 
-- Ordene os resultados pelo nome do projeto e, em seguida, pelo nome do cargo. Só inclua cargos que efetivamente tenham tarefas no respectivo projeto. 
select p.nome ,r.cargo,count(t.id)from Projetos as p join Tarefas as t on p.id = t.id_projeto join Responsaveis as r on r.id = t.id_responsavel_tarefa where p.status = 'Em Andamento' group by p.nome,r.cargo order by p.nome,r.cargo;

-- 5) Gere um ranking com os Top 5 projetos com mais tarefas em atraso.
 select p.nome,count(t.id) from Projetos as p join Tarefas as t on p.id = t.id_projeto where t.status != 'Concluída' group by p.id,p.nome order by count(t.id) desc limit 5;

-- 6) Verifique quais as Tarefas são urgentes/altas e não concluídas
select t.titulo,t.prioridade,t.status,p.nome,r.nome from Tarefas as t join Projetos as p on t.id_projeto = p.id join Responsaveis as r on r.id = t.id_responsavel_tarefa where (t.prioridade = 'Alta' or t.prioridade = 'Urgente') and t.status != 'Concluída';

-- 7) Listar projetos com uma ou mais tarefas com status 'Pendente'
select p.nome, count(t.id) from Projetos as p join Tarefas as t on p.id = t.id_projeto where t.status = 'Pendente' group by p.nome;