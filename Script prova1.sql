...
select * from curso as c join oferta as o on c.id_curso = o.id_curso where data_inicio between '2023-01-01' and '2023-06-30' order by data_inicio;

-- 2) Qual é o total de inscrições por tipo de público  alvo?
select count(id_usuario), publico_alvo from inscricao as i join oferta as o on i.id_oferta = o.id_oferta group by publico_alvo;

-- 3) Quantas inscrições existem por curso?
select count(id_inscricao),id_curso from inscricao as i join oferta as o on i.id_oferta = o.id_oferta group by id_curso order by id_curso;

-- 4) Liste os 10 usuários mais recentes que se inscreveram em ofertas.
select id_usuario,id_oferta,data_inscricao from inscricao order by data_inscricao desc limit 10;
...
-- Além disso, ordene os resultados pela média de carga horária em ordem decrescente e limite a lista aos 5 primeiros cursos
select avg(c.carga_horaria),c.nome,i.id_oferta,i.id_usuario,u.data_nascimento from inscricao as i join usuario as u on i.id_usuario = u.id_usuario join curso as c on c.id_curso = i.id_inscricao  where data_nascimento between '1980-01-01' and '2000-12-31' group by i.id_oferta,i.id_usuario,c.nome order by avg(carga_horaria) desc limit 5;

-- 6) Gere o CROSS JOIN das tabelas cursos e ofertas
select * from curso as c cross join oferta as o on c.id_curso = o.id_curso;
